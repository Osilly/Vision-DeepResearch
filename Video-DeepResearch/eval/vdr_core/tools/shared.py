...
async def call_extract_model_async(
    *,
    ...
    """Call a local extract model served by SGLang or an OpenAI-compatible endpoint (vLLM).

    Supported backends (set via EXTRACT_BACKEND env var, auto-bridged from
    config.yaml's ``extract_backend`` by env._sync_tool_config_to_env):
      - ``sglang_generate`` (aliases: ``sglang`` / ``generate`` / ``native``)
            → POST ``{url}/generate`` with sglang-native payload.
      - ``vllm`` / ``openai`` (default)
            → POST ``{url}/v1/chat/completions`` with OpenAI-compatible payload
              (works for vLLM, OpenAI, and any other OpenAI-compatible gateway).
    """
    try:
        import requests
    except ImportError:
    ...
    elif backend in {"vllm", "openai_compatible", ""}:
        backend = "openai"
