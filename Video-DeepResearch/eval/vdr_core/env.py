...
_SYSTEM_PROMPT_PATH = Path(__file__).parent.parent / "prompts" / "eval_system_prompt.txt"
...
class DeepResearchEnv(BaseInteractionEnv):
    ...
    async def _judge(self, response_text: str) -> float:
        ...
        For eval usage: run_eval.py computes reward via DeepResearchReward
        directly, not via env — so the slime.rollout.rm_hub import is optional.
        If unavailable, we return 0.0 (eval scoring is unaffected).
        """
        try:
    ...
    def _visit(self):
        if self._visit_tool is None:
            from .tools.visit_tool import VisitTool
    ...
    def _crop_search(self):
        if self._crop_and_search_tool is None:
            from .tools.crop_and_search_tool import CropAndSearchTool
    ...
    def _select_crop_search(self):
        if self._select_crop_search_tool is None:
            from .tools.select_crop_search_tool import SelectCropSearchTool
