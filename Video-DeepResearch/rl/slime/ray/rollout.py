...
class RolloutManager:
    ...
    def _convert_samples_to_train_data(self, samples: list[Sample] | list[list[Sample]]):
        ...
        if samples[0].teacher_topk_indices is not None:
...
def _log_eval_rollout_data(rollout_id, args, data, extra_metrics: dict[str, Any] | None = None):
    ...
    _save_eval_trace_jsonl(rollout_id, step, args, data)

    return log_dict


def _extract_answer_from_response(response: str | None) -> str:
    if not response:
        return ""
...
def _save_eval_trace_jsonl(rollout_id: int, step: int, args, data: dict[str, dict[str, Any]]) -> None:
    save_dir = getattr(args, "eval_trace_save_dir", None)
    if not save_dir:
        return

    path = Path(save_dir) / f"eval_rollout_{rollout_id}_step_{step}.jsonl"
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with path.open("w", encoding="utf-8") as f:
        for dataset_name, info in data.items():
            ...
            for idx, sample in enumerate(samples):
                ...
                judge_result = rewards[idx] if idx < len(rewards) else sample.reward
                ...
                record = {
                    ...
                    "question": _jsonable(question),
                    "answer": _jsonable(answer),
                    "trajectory": _jsonable(trajectory),
                    "final_answer": _extract_answer_from_response(sample.response),
                    "judge_result": _jsonable(judge_result),
                }
                f.write(json.dumps(record, ensure_ascii=False) + "\n")

    logger.info(f"Saved eval traces to {path}")
