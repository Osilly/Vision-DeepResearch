...
from .cp_utils import get_logits_and_tokens_offset_with_cp, slice_log_prob_with_cp, slice_with_cp
...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def _get_rollout_data(self, rollout_data_ref: Box) -> RolloutBatch:
        ...
        def _slice_response_matrix_with_cp(
            value,
            total_length: int,
            response_length: int,
            dtype: torch.dtype,
            ...
            tensor = torch.tensor(value, device=torch.cuda.current_device(), dtype=dtype)
            assert tensor.size(0) == response_length, f"{tensor.size(0)} vs {response_length}"
        ...
        for key, dtype in [("teacher_topk_log_probs", torch.float32), ("teacher_topk_indices", torch.long)]:
            if key not in rollout_data:
                continue
            rollout_data[key] = [
                _slice_response_matrix_with_cp(
                    value,
                    total_length,
                    response_length,
                    dtype,
