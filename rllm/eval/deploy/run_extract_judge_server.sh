#!/bin/bash
# Shared extract + judge server — serves both the summary role (consumed by
# VisitTool & crop_and_search) and the judge / reward role (consumed by
# eval_runner.py). A single VL instruct model covers both.
#
